...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:admin@192.168.0.225:3306/class_history'
db = SQLAlchemy(app)
...
class Subject(db.Model):
    ...
    name = db.Column(db.String(255), nullable=False)
    branch_id = db.Column(db.Integer, db.ForeignKey('branch.branch_id'))
    subject_details = db.relationship('SubjectDetail', back_populates='subject')
...
class SubjectDetail(db.Model):
    ...
    branch_id = db.Column(db.Integer, db.ForeignKey('branch.branch_id'))
    subject = db.relationship('Subject', back_populates='subject_details')

class Lesson(db.Model):
    __tablename__ = 'lessons'
    lesson_id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    subject_detail_id = db.Column(db.Integer, db.ForeignKey('subject_detail.subject_detail_id'))
